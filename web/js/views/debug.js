...
function routeChipHtml(t) {
    ...
    const parts = [];
    if (d.source)
        parts.push(String(d.source));
    if (d.intent)
        parts.push(String(d.intent));
...
export function createDebugView() {
    ...
    function mergeSpan(span) {
        ...
        if (!t) {
            ...
            if (i >= 0)
                ...
                t.spans = [...t.spans, span].sort((a, b) => a.started_at - b.started_at);
        }
    ...
    function renderWaterfall() {
        ...
        const model = selected.spans.map((s) => s.data?.model).find((m) => typeof m === "string") ??
            "—";
      ...
      <span class="muted">${selected.spans.length} spans</span>`;
        ...
        for (const sp of selected.spans) {
            ...
            const modelTag = typeof sp.data?.model === "string" ? ` · ${sp.data.model}${gpuTag}` : "";
            row.innerHTML = `
    ...
    function renderDetail() {
        ...
        for (const tab of ["data", "prompt", "response"]) {
