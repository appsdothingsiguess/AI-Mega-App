...
export type ModelsOpts = {
  ...
  onDrafts: (next: ModelDraft[]) => void;
  onBaseline: (next: ModelDraft[]) => void;
  onSwapYaml: (yaml: string) => void;
};

function gpuValue(g: GpuId): string {
  return g === "cpu" ? "cpu" : String(g);
}

function parseGpu(v: string): GpuId {
  if (v === "cpu") return "cpu";
  const n = Number(v);
  return Number.isFinite(n) ? n : "cpu";
}

function same(a: ModelDraft, b: ModelDraft): boolean {
  return (
    a.gpu === b.gpu &&
...
function dirtyList(drafts: ModelDraft[], baseline: ModelDraft[]): ModelDraft[] {
...
export function mountModelsPanel(panel: HTMLElement, opts: ModelsOpts): ModelsHost {
  let applying = false;
  ...
  const render = () => {
    ...
    const rows = opts.drafts
      .map((d, i) => {
        ...
        const optsHtml = devices
          .map(
            (o) =>
              `<option value="${escapeHtml(o.value)}"${gpuValue(d.gpu) === o.value ? " selected" : ""}>${escapeHtml(o.label)}</option>`,
          )
          .join("");
    ...
    const pre = panel.querySelector<HTMLElement>(".swap-config");
    if (pre) pre.textContent = opts.swapYaml || "(empty)";

    panel.querySelectorAll<HTMLSelectElement>(".field-device").forEach((sel) => {
      sel.addEventListener("change", () => {
        const i = Number(sel.dataset.i);
        const next = opts.drafts.map((d, idx) =>
          idx === i ? { ...d, gpu: parseGpu(sel.value) } : d,
        );
        opts.onDrafts(next);
        opts.drafts = next;
        render();
    ...
    panel.querySelectorAll<HTMLButtonElement>("[data-act=resident]").forEach((btn) => {
      btn.addEventListener("click", () => {
        const i = Number(btn.dataset.i);
        const next = opts.drafts.map((d, idx) =>
          idx === i ? { ...d, resident: !d.resident } : d,
        );
        opts.onDrafts(next);
        opts.drafts = next;
        render();
    ...
    panel.querySelector<HTMLButtonElement>(".btn-apply")?.addEventListener("click", () => {
      void apply();
    });
  };
  ...
  async function apply(): Promise<void> {
    ...
    try {
      ...
      opts.toast(
        ok ? `Applied — ${result.path || "swap-config updated"}` : "Apply returned ok=false",
        ok ? "ok" : "err",
      );
    } catch (e) {
