...
from app.model_scheduler import ModelScheduler, reset_model_scheduler
...
async def test_skip_when_already_loaded_main() -> None:
    settings = _settings()
    scheduler = ModelScheduler(settings)
    scheduler._loaded_main = "qwen3:8b"
...
async def test_unload_and_warmup_on_swap() -> None:
    settings = _settings()
    scheduler = ModelScheduler(settings)
    scheduler._loaded_main = "qwen2.5-coder:7b"
    ...
    assert unload_call.kwargs["json"] == {"model": "qwen2.5-coder:7b", "keep_alive": 0}
    assert warmup_call.kwargs["json"] == {
        "model": "qwen3:8b",
        "prompt": "",
        "keep_alive": -1,
    }
    assert scheduler._loaded_main == "qwen3:8b"
...
async def test_serialization_behind_lock() -> None:
    ...
    assert scheduler._loaded_main == "qwen2.5-coder:7b"
