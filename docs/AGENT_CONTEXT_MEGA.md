...
Intended (PLAN §4.1 Config B): gpu0-main swap group = [chat-default, coder, coder-small, vision] (one at a time on 3090); resident group = [dispatcher(GPU1), utility, embed, classifier (CPU)]; reasoner = chat-default blob w/ thinking (deduped, never a swap entry); dispatcher latency is critical path (5-7x degradation when GPU1 shared).
