...
Complete `.cursor/rules/` content for the rebuild. **These blocks are already live** in `.cursor/rules/` — `001-stack`, `002-boundaries` (always-apply) · `003-python-backend`, `004-frontend`, `005-config`, `006-testing`, `010-benchmark-eval-methodology`, `011-ui-design` (glob) · `007-git-worktrees`, `008-remote-box`, `009-subagents` (agent-requested).

**`010-benchmark-eval-methodology.mdc` is the one rule not derived from design preference.** Every line in it traces to a specific wrong number published during Phase 0 and later retracted — a scorer whose substring bug cost 45 accuracy points, a thinking-suppression convention that worked on exactly one checkpoint, token budgets that measured truncation and were read as model quality, a rubric that failed a model for something the shipping code fixes anyway, a textbook mitigation that measured worse, and two background evals racing for one port. It is glob-attached to `scripts/bench_*`, `scripts/eval_*`, and the measurement docs; read it in full before writing or trusting any local-model eval. Its content is not duplicated here — read the `.mdc`. Editing a rule means editing both the live `.mdc` and its block here — they must stay identical. Sections 8–11 below cover the rest of the Cursor 3 system (Skills, hooks, parallel-agent files, Plan Mode, model selection) that live *outside* `.cursor/rules/`.
