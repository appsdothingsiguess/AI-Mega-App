...
| `vision` | **Qwen3-VL-32B** + mmproj (recommended over Gemma-3-27b-it) | PC1 split (swap) | **Phase-0 benchmarked (2026-07-22):** both PASS correctness (3/3 object-count), but Qwen3-VL-32B has the latency edge — real-gen 21-43 tok/s vs Gemma-3-27b-it's 11-27 tok/s, because Gemma's mmproj tokenizes the test image into far more prompt tokens (276 vs 54) despite similar raw bench tok/s (38.4 vs 43.0). Pending a broader accuracy test beyond one image/prompt before fully locking. |
| `utility` | Qwen3-8B Q4 | PC2 3070 resident | Titles/summaries/compaction/memory reviewer/instant fallback (one LAN hop). |
| `coder-small` | **Qwen2.5-Coder-7B** Q4 (not "Qwen3-Coder-7B" — that model doesn't exist; Qwen3-Coder only ships as 30B-A3B and a 3B-active "Next" MoE) | PC2 3070 resident (confirmed fits, 6.0GB, auto layer-split, no `--tensor-split` needed) | Parallel/fallback coding lane. Phase-0 benchmarked: PASS, 124.8 tok/s bench / 113.8-120.8 tok/s real. |
| `embed` | nomic-embed-v2 / Qwen3-embedding | PC2 3070 resident | Batched; LAN hop tolerable. Phase-0: nomic-embed-text-v2-moe on CPU PASS, 11.7-23.5ms/call. |
| `classifier` | Qwen3-1.7B-class | CPU resident | Router (grammar-constrained). **Phase-0 finding: Qwen3-1.7B-Q8_0 fails as-is** — its "thinking" mode can't be suppressed, burns its whole completion budget on internal reasoning and never emits an answer (0/5 on CPU, 14-15 tok/s). Needs `enable_thinking=false`/chat-template fix or a different non-thinking small model before this slot is usable. |
...
- **External research round 2 (Perplexity, 2026-07-22) — surfaces named alternative candidates, doesn't reverse the verdict.** Two independent, methodology-transparent community benchmarks (a 21-model "judgment" benchmark and a 13-model LM Studio benchmark) rank small instruction-tuned models — not distilled dispatcher-only architectures like Needle — well above Needle-class accuracy: `Qwen3-0.6B` and `LFM2.5-1.2B` tie for #1 (0.880 Agent Score, judgment benchmark), and `Qwen3.5-4B` scores 97.5% tool-calling accuracy (LM Studio benchmark). A direct Reddit head-to-head (Needle 26M vs. Qwen3-0.6B, 50 queries/5 tiers, CPU) found **Needle still wins on narrow, unambiguous, fixed-toolset single-shot dispatch** (both accuracy and 4.4x speed) — consistent with our own numbers, and explicitly framed there as "Needle = dispatcher, Qwen3 = small chatbot with tool-calling," i.e. Needle's edge shrinks as schema complexity (multi-arg, regex-string args, semantically overlapping tool names) rises, which matches our own 0/10 results on `file_grep`/`file_read`/`web_search`. Two more candidates worth knowing about if the small-Qwen fallback is chosen over Needle: **FunctionGemma 270M** (Google) is the closest size/purpose analog to Needle and has an actual published finetune case study — 10-39% base accuracy → 90-97% after a Distil Labs knowledge-distillation finetune from a 120B teacher, the kind of real before/after recipe Needle itself still lacks; **Hammer** (MadeAgents, ICLR 2025 Spotlight, 1.5B-7B) is peer-reviewed, open-source, and ships a dedicated irrelevant-function-rejection dataset/eval suite (HammerBench) built for exactly the semantic-overlap failure class we hit. **Net effect: no change to the Phase-3 plan** — still re-run the real ≥120-examples/tool Needle finetune once the tool registry stabilizes and compare against a small-Qwen dispatcher — but if that comparison ever needs a documented non-Needle alternative or a "what would we finetune instead" answer, FunctionGemma 270M (has a finetune recipe) and Hammer (handles ambiguous/overlapping tool names) are the two candidates to try first, ahead of generic Qwen3-0.6B/LFM2.5-1.2B, which mainly win on ecosystem maturity rather than accuracy-on-hard-schemas.
