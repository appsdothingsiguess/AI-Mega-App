...
**2026-08-21/2026-08-24 addendum — roster table above is Phase-0-dated (2026-07-23) and no longer matches the live roster.** `chat-default`/`reasoner`/`coder` moved off `Qwen3.6-35B-A3B` onto `Qwen3.8-27B-UD-Q4_K_XL` on 2026-08-21 (thinking now genuinely wanted on this checkpoint, native MTP speculative decoding, `coder` via a symlinked file to stay a distinct swap-slot member — see `config.yaml`'s `models.chat-default`/`models.coder` comments). Ctx also moved: `chat-default`/`coder` ship at **131072** (not this table's 32768/16384 — 262144 OOMs under the MTP flag set, confirmed on isolated re-test 2026-08-23), `reasoner` at 32768, `coder-small` (`Qwen2.5-Coder-7B`) at **30000** (native ceiling 32768, was mistakenly 8192 in earlier config), `vision` confirmed to stay at 8192 after testing its full 262144 ceiling (throughput-bound, not capacity-bound — 8192 is the sweet spot), `reasoner-alt` unchanged at 8192 and **still untested in any live sweep**. Current authoritative values: `config.yaml` directly; full narrative: `docs/HANDOFF.md`'s 2026-08-23/2026-08-24 entries and `docs/AGENT_CONTEXT_MEGA.md`.
...
**Phase 1 — Skeleton with eyes. Backend merged to `main` 2026-07-25** (config, SQLite schema, `llm_client`, chat SSE endpoint, debug trace store + API, CI/test harness — 42 tests passing). **Frontend (`web/`) still open**, being built in Cursor. Exit: chat with any manually-picked model, every turn fully traced — not reached until `web/` lands and the exit demo runs.
